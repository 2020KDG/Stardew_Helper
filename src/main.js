...
function renderAchievements() {
  ...
  let list = ACHIEVEMENTS.map(ach => {
    ...
    if (!isCleared) {
      // Money Fallbacks
      if (ach.id === 0 && totalMoney >= 15000) isCleared = true; // 풋내기
      if (ach.id === 1 && totalMoney >= 50000) isCleared = true; // 소몰이
      if (ach.id === 2 && totalMoney >= 250000) isCleared = true; // 농가 주인
      if (ach.id === 3 && totalMoney >= 1000000) isCleared = true; // 백만장자
      if (ach.id === 4 && totalMoney >= 10000000) isCleared = true; // 전설
      
      // House Upgrades
      if (ach.id === 18 && houseUpgradeLevel >= 1) isCleared = true; // 출세
      if (ach.id === 19 && houseUpgradeLevel >= 2) isCleared = true; // 크게 살기

      // Museum
      if (ach.id === 28 && museumPieces >= 40) isCleared = true; // 보물창고
      if (ach.id === 5 && museumPieces >= 95) isCleared = true; // 완성된 컬렉션

      // Quests
      if (ach.id === 29 && questsCompleted >= 10) isCleared = true; // 심부름꾼
      if (ach.id === 30 && questsCompleted >= 40) isCleared = true; // 큰 도움

      // Fish
      if (ach.id === 27 && fishCaught >= 100) isCleared = true; // 낚시의 신

      // Cooking
      if (ach.id === 15 && recipesCooked >= 10) isCleared = true; // 요리사
      if (ach.id === 16 && recipesCooked >= 25) isCleared = true; // 수셰프
      if (ach.id === 17 && recipesCooked >= 80) isCleared = true; // 미식가 셰프

      // Crafting
      if (ach.id === 20 && craftingRecipes >= 15) isCleared = true; // 수작업
      if (ach.id === 21 && craftingRecipes >= 30) isCleared = true; // 장인

      // Friendship (5 Hearts)
      if (ach.id === 6 && friendshipFiveHearts >= 1) isCleared = true; // 새 친구
      if (ach.id === 11 && friendshipFiveHearts >= 4) isCleared = true; // 무리
      if (ach.id === 12 && friendshipFiveHearts >= 10) isCleared = true; // 인맥 형성
      if (ach.id === 13 && friendshipFiveHearts >= 20) isCleared = true; // 인기인

      // Friendship (10 Hearts)
      if (ach.id === 7 && friendshipTenHearts >= 1) isCleared = true; // 절친
      if (ach.id === 9 && friendshipTenHearts >= 8) isCleared = true; // 총애 받는 농부

      // Story Flags
      if (ach.id === 101 && mailReceived.includes("ccIsComplete")) isCleared = true; // Local Legend
      if (ach.id === 102 && 
        ...
        isCleared = true; // Joja Co. Member Of The Year
      }
      if (ach.id === 103 && maxStamina >= 508) isCleared = true; // Mystery Of The Stardrops (7 stardrops = 508 stamina)
      if (ach.id === 104 && spouse !== "" && childrenCount >= 2) isCleared = true; // Full House
      if (ach.id === 105 && (farmingLevel >= 10 || miningLevel >= 10 || combatLevel >= 10 || foragingLevel >= 10 || fishingLevel >= 10)) isCleared = true; // Singular Talent
      if (ach.id === 106 && farmingLevel >= 10 && miningLevel >= 10 && combatLevel >= 10 && foragingLevel >= 10 && fishingLevel >= 10) isCleared = true; // Master Of The Five Ways
      if (ach.id === 107 && checkMonsterEradicationGoals(specificMonstersKilled)) isCleared = true; // Protector Of The Valley
      if (ach.id === 23 && currentSaveData && currentSaveData.has_skull_key) isCleared = true; // The Bottom
    }
...
let currentBundleFilter = 'all'; // 'all', 'progress', 'completed'

function renderBundles() {
  if (!bundlesListEl) return;
  
  // bundlesMap: { 0: [true, false, true...], 1: [...] }
  ...
  BUNDLES_DATA.forEach(bundle => {
    ...
    // Check how many items are true
    bundle.items.forEach((_, idx) => {
      if (savedStatus[idx] === true) completedItems++;
    });

    // Use reqCount if available, otherwise totalItems
    const targetCount = bundle.reqCount || totalItems;

    // Determine status based on targetCount
  ...
  // 2. 전체 번들 기준 방(Room)별 완료율 계산 (원본 데이터 기준)
  BUNDLES_DATA.forEach(bundle => {
...
function renderJojaBundles() {
  ...
  BUNDLES_DATA.forEach(bundle => {
    ...
    // We overwrite savedStatus array based on isJojaCompleted because Joja bundles 
    // are not tracked in the `bundles` map like the Community Center.
  ...
  for (const [roomName, roomData] of Object.entries(rooms)) {
    const roomDiv = document.createElement('div');
    roomDiv.className = 'bundle-room';
    roomDiv.id = `room-${roomName.replace(/ /g, '-')}`; // Add ID for scroll matching
...
function setupNavigation() {
  ...
  navBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // 1. 아코디언 제어 (Auto Collapse 다른 메뉴)
      allSubNavs.forEach(nav => nav.classList.remove('expanded'));

      // 클릭한 메뉴의 서브메뉴를 항상 연다 (활성 페이지와 싱크)
      if (targetSubNav) {
        ...
        if (targetSection) {
          ...
          setTimeout(() => {
            ...
            if (titleEl) {
              ...
              void titleEl.offsetWidth; // 브라우저 리플로우 강제로 트리거하여 애니메이션 초기화
              titleEl.classList.add('flash-highlight');
              
              // 애니메이션 완료 후 클래스 제거 (css에서 1초)
    ...
    mainScrollArea.addEventListener('scroll', () => {
      ...
      scrollTimeout = requestAnimationFrame(() => {
        ...
        sections.forEach(div => {
          ...
          // 화면의 상단(top: 0 근처)부터 어느 섹션이 가장 가까운지 확인
          const distance = Math.abs(rect.top - 120); // 120px offset for sticky header
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (refreshIconBtn) {
    refreshIconBtn.addEventListener("click", () => {
      ...
      // Optional: Add spinning animation class temporarily
